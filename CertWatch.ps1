...
<#
.SYNOPSIS
    CertWatch - סורק תעודות SSL/TLS בסגמנטי רשת
.DESCRIPTION
    סורק טווח IP, בודק אילו שרתים פעילים, ושולף תעודות
    מ-LocalMachine\My (Personal) דרך PowerShell Remoting (WinRM)
.NOTES
    אין צורך בהתקנות - רץ על כל Windows עם PowerShell 5.1+
    הרץ כ-Administrator לתוצאות הטובות ביותר
#>
...
function Get-CertsFromRemote {
    param([string]$IP, [string]$Username, [string]$Password, [bool]$UseCurrentUser)
    ...
    try {
        if ($UseCurrentUser) {
            $certs = Invoke-Command -ComputerName $IP -ScriptBlock $script `
                                    -ErrorAction Stop
        } else {
            $secPass = ConvertTo-SecureString $Password -AsPlainText -Force
            $cred    = New-Object System.Management.Automation.PSCredential($Username, $secPass)
            $certs   = Invoke-Command -ComputerName $IP -Credential $cred `
                                      -ScriptBlock $script -ErrorAction Stop
        }
        return $certs
    } catch {
        return $null
...
function Get-CertStatus {
    ...
    if     ($DaysLeft -lt 0)        { return @{ Label="פג תוקף";      Tag="expired";  Color=$TextMuted } }
    elseif ($DaysLeft -le $Critical){ return @{ Label="קריטי";        Tag="critical"; Color=$Red      } }
    elseif ($DaysLeft -le $Warning) { return @{ Label="אזהרה";        Tag="warning";  Color=$Yellow   } }
    elseif ($DaysLeft -le $Notice)  { return @{ Label="שים לב";       Tag="notice";   Color=$Blue     } }
    else                            { return @{ Label="תקין";          Tag="ok";       Color=$Green    } }
}

# ─── בניית ממשק ──────────────────────────────────────────

$Form = New-Object System.Windows.Forms.Form
$Form.Text            = "CertWatch — סורק תעודות ברשת"
...
$SubLbl.Text      = "סורק תעודות SSL/TLS בסגמנטי רשת  |  PowerShell Edition"
$SubLbl.ForeColor = $TextMuted
$SubLbl.AutoSize  = $true
$SubLbl.Location  = New-Object System.Drawing.Point(220, 20)
...
$cy = 10
$cy = Add-SectionLabel $lp "🌐  סריקת רשת" $cy
$cy += 10
$cy = Add-Label $lp "טווח IP  (למשל 192.168.1.1-254)" $cy
$IpRangeBox = Add-TextBox $lp "192.168.1.1-254" $cy; $cy += 28
$cy = Add-Label $lp "Timeout ping (שניות)" $cy
$PingTimeout = Add-NumericUpDown $lp 1 1 10 $cy; $cy += 30
$cy = Add-Label $lp "Threads מקביליות" $cy
$ThreadsNum = Add-NumericUpDown $lp 30 1 100 $cy; $cy += 30

$cy = Add-SectionLabel $lp "🔑  WinRM / אישורים" ($cy + 6)
$cy += 10

$UseCurrentChk = New-Object System.Windows.Forms.CheckBox
$UseCurrentChk.Text      = "משתמש נוכחי (Domain)"
...
$cy = Add-SectionLabel $lp "⚠️  ספי התראות (ימים)" ($cy + 6)
$cy += 10

foreach ($cfg in @(
    @{Label="🔴  קריטי";      Default=30},
    @{Label="🟡  אזהרה";      Default=60},
    @{Label="🔵  תשומת לב";  Default=90}
)) {
    $cy = Add-Label $lp $cfg.Label $cy
    $n = Add-NumericUpDown $lp $cfg.Default 1 365 $cy
    switch ($cfg.Label) {
        {$_ -match "קריטי"} { $Global:CritNum   = $n }
        {$_ -match "אזהרה"} { $Global:WarnNum   = $n }
        {$_ -match "תשומת"} { $Global:NoticeNum = $n }
    }
    $cy += 30
}
...
foreach ($f in @(
    @{Text="הכל";         Val="all";      Color=$TextLight},
    @{Text="🔴 קריטי";    Val="critical"; Color=$Red},
    @{Text="🟡 אזהרה";    Val="warning";  Color=$Yellow},
    @{Text="🔵 שים לב";   Val="notice";   Color=$Blue},
    @{Text="✅ תקין";     Val="ok";       Color=$Green}
)) {
...
$SummaryText.Text      = "`n  הרץ סריקה כדי לראות סיכום..."
...
function Apply-Filter {
    ...
    foreach ($r in $Global:AllCertRows) {
        ...
        if ($search) {
            $match = $false
            foreach ($sv in $r.SubItems) { if ($sv.Text.ToLower().Contains($search)) { $match = $true; break } }
        }
...
function Update-Summary {
    ...
    $Form.Invoke([Action]{
        ...
        $SummaryText.AppendText("`n  📊  סיכום סריקה`n`n")

        $lines = @(
            @{ Label="  שרתים פעילים  ";  Val=$hosts;   Color=$Blue    },
            @{ Label="  פג תוקף         "; Val=$expired; Color=$TextMuted},
            @{ Label="  🔴  קריטי       "; Val=$crit;   Color=$Red     },
            @{ Label="  🟡  אזהרה       "; Val=$warn;   Color=$Yellow  },
            @{ Label="  🔵  תשומת לב   "; Val=$notice; Color=$Blue    },
...
function Start-Scan {
    try { $ips = Parse-IPRange $IpRangeBox.Text }
        ...
        [System.Windows.Forms.MessageBox]::Show("טווח IP לא תקין: $_", "שגיאה")
        return
    }
    ...
    $ProgressBar.Value   = 0
    Set-Status "סורק..." $Yellow
    Write-Log "מתחיל סריקה של $($ips.Count) כתובות IP" "info"
    ...
    $collector = {
        ...
        foreach ($job in $jobs) {
            ...
            $job.PS.Dispose()
            $done++
            if ($result -eq $null) { Update-Progress $done $total; continue }

            $Global:ScanResults.Add($result)
            Write-Log "✓  $($result.IP)  ($($result.Hostname))  — $($result.Certs.Count) תעודות" "ok"

            if ($result.Certs.Count -eq 0) {
                $st = Get-CertStatus 9999 $critDays $warnDays $noticeDays
                Add-CertRow $result.IP $result.Hostname "(אין תעודות)" "" "" "" "—" "ok" $Green
            } else {
        ...
        Write-Log "✅  סריקה הסתיימה — $($Global:ScanResults.Count) שרתים | $crit קריטי | $warn אזהרה" "ok"
        Set-Status "הסתיים | $crit קריטי | $warn אזהרה" $(if ($crit -gt 0) { $Red } else { $Green })
        $Form.Invoke([Action]{
...
$StopBtn.Add_Click({
    $Global:IsRunning = $false
    Set-Status "עצר" $Yellow
    Write-Log "הסריקה נעצרה" "warn"
    $ScanBtn.Enabled = $true
    $StopBtn.Enabled = $false
})
...
$ExportBtn.Add_Click({
    if ($Global:AllCertRows.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("אין נתונים לייצוא", "ייצוא")
        return
    }
    ...
    if ($dlg.ShowDialog() -eq "OK") {
        ...
        Write-Log "✅  ייצוא CSV נשמר: $($dlg.FileName)" "ok"
        [System.Windows.Forms.MessageBox]::Show("הקובץ נשמר:`n$($dlg.FileName)", "ייצוא")
    }
})

# ─── הפעלה ────────────────────────────────────────────────
Write-Log "CertWatch מוכן — PowerShell Edition" "info"
Write-Log "הגדר טווח IP ולחץ התחל סריקה" "info"
